...
format:e.RGBAFormat});g.scissorTest=!0;g.texture.generateMipmaps=!1;var k=new e.Vector2(.441,.156),p=(new e.PlaneBufferGeometry(1,1,10,20)).deleteAttribute("normal").toNonIndexed(),m=p.attributes.position.array,l=p.attributes.uv.array;p.attributes.position.count*=2;p.attributes.uv.count*=2;var v=new Float32Array(2*m.length);v.set(m);v.set(m,m.length);var q=new Float32Array(2*l.length);q.set(l);q.set(l,l.length);l=new e.Vector2;m=m.length/3;for(var f=0,D=v.length/3;f<D;f++){l.x=v[3*f];l.y=v[3*f+1];
...
getScreenVector:function(a){a=a.clone();var b=this.container.clientWidth/2,c=this.container.clientHeight/2;a.project(this.camera);a.x=a.x*b+b;a.y=-(a.y*c)+c;a.z=0;return a},checkSpriteInViewport:function(a){this.camera.matrixWorldInverse.copy(this.camera.matrixWorld).invert();this.cameraViewProjectionMatrix.multiplyMatrices(this.camera.projectionMatrix,this.camera.matrixWorldInverse);this.cameraFrustum.setFromProjectionMatrix(this.cameraViewProjectionMatrix);return a.visible&&this.cameraFrustum.intersectsSprite(a)},
